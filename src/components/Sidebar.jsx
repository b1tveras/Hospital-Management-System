  ...
  return (
    <aside className="w-64 bg-white/70 dark:bg-slate-900/70 backdrop-blur-xl border-r border-white/50 dark:border-slate-800/50 flex flex-col h-full shadow-lg shadow-blue-900/5 dark:shadow-blue-900/20 z-20 transition-colors duration-200">
      <div className="h-20 flex flex-col justify-center px-6 border-b border-slate-200/50 dark:border-slate-800/50 shrink-0">
        ...
        <span className="text-[10px] text-slate-500 dark:text-slate-400 uppercase tracking-widest mt-1.5 font-semibold pl-[3.25rem]">
          Care. Cure. Comfort.
        </span>
      </div>
          ...
          {navItems.map((item) => {
            ...
            return (
              <li key={item.path}>
                ...
                >
                  <Icon
                    className={`h-5 w-5 transition-colors duration-300 ${isActive ? "text-white" : "text-slate-400 group-hover:text-blue-600 dark:group-hover:text-blue-400"}`}
                  />
                  {item.label}
                </Link>
              </li>
