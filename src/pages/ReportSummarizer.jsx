    ...
    try {
      ...
      const content = isPDF
        ? [{ type: "document", source: { type: "base64", media_type: mediaType, data: base64 } },
           { type: "text", text: `Analyze this medical report. Respond in ${lang}.` }]
      ...
      {result && (
        <div className="mt-6 space-y-4">
          ...
          <div className="bg-green-50 border border-green-200 rounded-lg p-4">
            <p className="text-xs font-semibold text-gray-500 uppercase mb-2">💬 Simple Summary</p>
          ...
          <div className="grid grid-cols-2 gap-3">
            {result.findings?.map((f, i) => (
              <div key={i} className="bg-white border rounded-lg p-3">
                ...
                <p className="text-xl font-bold">{f.value} <span className="text-sm font-normal text-gray-400">{f.unit}</span></p>
                <span className={`text-xs px-2 py-0.5 rounded-full ${f.status === "high" ? "bg-red-100 text-red-600" : f.status === "low" ? "bg-yellow-100 text-yellow-700" : "bg-green-100 text-green-700"}`}>
          ...
          <div className="bg-white border rounded-lg p-4">
            <p className="text-xs font-semibold text-gray-500 uppercase mb-2">👨⚕️ Doctor Ki Salah</p>
          ...
          <p className="text-xs text-yellow-700 bg-yellow-50 border-l-4 border-yellow-400 p-3 rounded">
            ⚠️ Ye AI summary sirf samajhne ke liye hai. Ilaaj ke liye doctor se milein.
          </p>
        </div>
      )}
    </div>
